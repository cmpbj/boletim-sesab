...
REGEXP_NOVOS_CASOS = re.compile(
    "Nas últimas 24 horas, foram registrados ([0-9.]+) casos de Covid-19")
REGEXP_NOVAS_MORTES = re.compile("mais ([0-9.]+) ó")
...
REGEXP_VACINACAO_UMA_DOSE = re.compile(
    "([0-9.]+) pessoas vacinadas com a primeira dose")
REGEXP_VACINACAO_DUAS_DOSES = re.compile(
    "([0-9.]+) com a segunda dose ou dose única")
REGEXP_VACINACAO_DOSE_REFORCO = re.compile("([0-9.]+) com a dose de reforço")
REGEXP_VACINACAO_SEGUNDO_REFORCO = re.compile(
    "([0-9.]+) com o segundo reforço")
REGEXP_VACINACAO_CRIANCAS = re.compile(
    "([0-9.]+) crianças já foram imunizadas")
REGEXP_VACINACAO_CRIANCAS_SEGUNDA_DOSE = re.compile(
    "([0-9.]+) já tomaram também a segunda dose")
...
data_do_arquivo = '{}-{}-{}'.format(data_atual.day,
...
try:
    titulo = navegador.find_element(
        by=By.PARTIAL_LINK_TEXT, value='casos ativos')
    print(titulo.text)

except NoSuchElementException:
    try:
        titulo = navegador.find_element(
            by=By.PARTIAL_LINK_TEXT, value='não registra')
    except NoSuchElementException:
        titulo = navegador.find_element(
            by=By.PARTIAL_LINK_TEXT, value='sem registro')
finally:
    print(titulo.text)
...
if data_em_texto in data_hora.text:
    ...
    vacina_duas_doses = list(
        set(REGEXP_VACINACAO_DUAS_DOSES.findall(conteudo)))
    vacina_reforco = list(set(REGEXP_VACINACAO_DOSE_REFORCO.findall(conteudo)))
    vacina_segundo_reforco = list(
        set(REGEXP_VACINACAO_SEGUNDO_REFORCO.findall(conteudo)))
    vacina_criancas = list(set(REGEXP_VACINACAO_CRIANCAS.findall(conteudo)))
    vacina_criancas_segunda_dose = list(
        set(REGEXP_VACINACAO_CRIANCAS_SEGUNDA_DOSE.findall(conteudo)))
else:
...
documento.add_paragraph(
    f'Opção de título 01: Bahia registra {novos_casos[0]} novos casos de Covid-19 e {novas_mortes[0]} mortes em 24 horas')

documento.add_paragraph(
    f'Opção de subtítulo 01: O boletim epidemiológico divulgado pela Sesab {dia} ({dia_da_semana}) mostra que a Bahia atingiu {casos_ativos[0]} casos ativos da doença')

documento.add_paragraph(
    f'Opção de título 02: Bahia registra {casos_ativos[0]} casos ativos de Covid-19 e {novas_mortes[0]} mortes pela doença')

documento.add_paragraph(
    f'Opção de subtítulo 02: Ao todo, são {casos[0]} casos confirmados desde o início da pandemia')
...
documento.add_paragraph(
    f'No estado, atualmente, são {casos_ativos[0]} casos ativos do novo coronavírus, além de {casos_descartados[0]} descartados e {casos_investigacao[0]} em investigação.')
